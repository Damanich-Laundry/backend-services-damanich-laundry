...
class UserController {
  ...
  async updateStatus(req, res, next) {
    try {
      const user = await userUsecase.updateStatus(req.params.id);
      if (!user) return res.status(404).json({ message: "User not found" });
      res.json(user);
    } catch (err) {
      next(err);
  ...
  async getProfile(req, res, next) {
    try {
      const user = await userUsecase.getProfile(req.user.id);
  ...
  async delete(req, res, next) {
    try {
      const user = await userUsecase.deleteUser(req.params.id);
      if (!user)
